...
\begin{frame}[fragile]
  ...
  \begin{tikzpicture}[remember picture,overlay]
	...
	\node[notice={(-3cm, -0.7cm)}, text width=5cm, align=left] at ($(end1) + (5cm, 1.5cm)$) {\tiny \color{blue}{ 
			蓝色的外层循环为相互独立的蒙特卡洛模拟次数并行度高, 可并行在单个MIC的不同线程上也可以并行在多个MIC处理器上}};
	\node[notice={(-4.5cm, 0.4cm)}, text width=5cm, align=left] at ($(end2) + (6.5cm, -0.5cm)$) {\tiny \color{red}{ 
