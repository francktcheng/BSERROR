...
\begin{itemize}
\item 给定$N$, 计算$M_T^N$
\item 给定$M$, 计算$M_T^N\leq \epsilon$的概率，$\epsilon$为我们可以接受的最大误差
\item 由计算出的概率更新$M, N$ 重复前两步，直到$M_T^N \leq \epsilon$的概率刚好超过某一特定值：$Prob=95\%$
\end{itemize}
如何选择$M, N$的初始值以及如何迭代更新？
\begin{itemize}
\item 较大的$M$会增大模特卡罗方法计算出来的概率的可信度
\item 较大的$M$将不可避免的造成更高的资源和硬件的要求
\item 过小的$M$可能给出错误的$N$的更新方向，导致不收敛
\end{itemize}
...
\begin{theorem}
 ...
 T^{\frac{1}{4}}(\frac{\sigma}{(\log(\frac{x_0}{K})+\frac{1}{2}\sigma^2T)\sqrt{2\pi}})^{\frac{1}{2}}e^{-\frac{1}{4}}e^{-\frac{3\gamma^{\frac{2}{3}}\pi^{\frac{1}{3}}}{2ex_0^{\frac{2}{3}}T^{\frac{1}{3}}16^{\frac{1}{3}}e^{\frac{\sigma^2}{3}}}n^{\frac{1}{3}}}, otherwise\\
 \end{cases}
 \end{equation}
\end{theorem}
\end{frame}
...
我们给出参数迭代的算法：
\begin{algorithm}[H]
\textbf{Step1}:由上面定理确定了$N$的迭代初始值之后$N_0$

\textbf{Step2}:固定该$N_0$, 选取尽可能小的$M$的值$M_0$,使得模特卡罗模拟出的概率$\mathbb{P}(M_T^N<\epsilon)$收敛稳定

\textbf{Step3}:固定$M_0$, 使用二分法在$[0, N_0]$区间内寻找最优的$N$。 首先我们令$N_1=N_0/2$， 在此参数下模拟相应的
